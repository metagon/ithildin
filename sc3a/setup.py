...
def setup():
    ...
    # This is an issue that arises when mythril is used as a library and tries to acces ~/.mythril/signatures.db
    # Related issue: https://github.com/metagon/sc3a/issues/1
    myth_home = os.path.join(os.path.expanduser('~'), '.mythril')
    if os.path.exists(myth_home):
        log.debug('Mythril home directory exists')
    else:
